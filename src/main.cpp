...
#if defined(NDEBUG)
#error "Acre cannot be compiled without assertions."
#endif
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Acre
...
{
    RenameThread("acre-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZACREMinted()
{
...
void RecalculateZACRESpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zACRE supply
...
bool RecalculateACRESupply(int nHeightStart)
{
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert ACRE to zACRE
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Acre: We use certain sporks during IBD, so check to see if they are
